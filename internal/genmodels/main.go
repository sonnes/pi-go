...
// statusDeprecated is the models.dev marker for a model the vendor has
// retired. Emitting one hands callers a spec the API will reject, so
// deprecation upstream removes it from the generated catalog here.
const statusDeprecated = "deprecated"

// generate returns formatted Go source for a provider package and the
// number of models emitted. Every output modality is represented — text,
// image, audio and video alike — so the catalog mirrors what the vendor
// currently offers. Dated snapshots and deprecated models are skipped.
