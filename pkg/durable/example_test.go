...
// them but does not run them. The compiler therefore makes sure that
// the API shape is correct while the implementation is still landing.
...
// OrderQuery and OrderStatus are the typed input and output of the
// example tool.
...
// newAssistant returns the example [ai.LanguageModel]. It is a fixed
// model bound to a mock provider that replies with canned text. The
// examples therefore run without network access.
...
// Example shows the full flow: a session addressed by ID and resumed
// across processes.
func Example() {
	...
	// what it means: a ticket number, a user ID, or a thread key. The
	// same ID always resumes the same conversation. Agent options such
...
// ExampleNew shows durability: the same session ID continues the
// conversation from where it stopped, even across process restarts.
func ExampleNew() {
	store := session.NewMemoryStore()
	ctx := context.Background()
...
// ArtifactEntry is an application-defined entry. It persists in the
// transcript tree, and it never reaches the model.
...
// ExampleAgent_Append shows how to persist custom application entries
...
// ExampleAgent_Branch shows in-place branching. Move the leaf, and the
// next turn grows a sibling. The abandoned branch stays in the tree.
...
// ExampleAgent_Fork shows how to lift the active path into a separate
...
// ExampleAgent_Compact shows how to shrink the model context without
// the removal of history. The agent appends a summary entry, and the
// context builder does the rest.
func ExampleAgent_Compact() {
	...
	// turns. Nothing is removed, so rewind still works.
...
// ExampleAgent_Run shows streaming. The run stream is turn-scoped. It
// carries lifted inner agent events, with persistence receipts on the
...
func ExampleAgent_Run() {
	...
	for e, err := range s.Events() {
		...
		case e.Agent.Type == agent.EventMessageEnd:
			// Receipt: the entries of the message are in the store.
...
// mutations commit. The application decides whether to log them,
// forward them, or fan them out.
