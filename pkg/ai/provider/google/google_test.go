...
// newTestProvider creates a Google provider configured for testing.
func newTestProvider(t *testing.T) (*google.Provider, func()) {
	t.Helper()
	...
	if recording, _ := httprr.Recording(trace); recording {
		...
		if apiKey == "" {
			t.Skip("GOOGLE_API_KEY not set, skipping recording")
		}
	}

	p, err := google.New(
		google.WithAPIKey(apiKey),
		google.WithHTTPClient(rr.Client()),
		google.WithToolCallIDFunc(func() string { return "test-tool-call-id" }),
	)
	if err != nil {
		rr.Close()
