...
// TokenEndpoint is the default Google OAuth token endpoint.
const TokenEndpoint = "https://oauth2.googleapis.com/token"

// Refresher implements [oauth.TokenRefresher] for Google OAuth.
type Refresher struct {
	// Client is the HTTP client used for token requests.
	// If nil, [http.DefaultClient] is used.
	Client *http.Client
	// TokenURL overrides the default Google token endpoint.
...
func (r *Refresher) RefreshToken(ctx context.Context, creds oauth.Credentials) (oauth.Credentials, error) {
	...
	if r.ClientSecret == "" {
		return oauth.Credentials{}, fmt.Errorf("oauth: ClientSecret is required")
	}
	client := r.Client
	if client == nil {
...
// NewOAuthTransport creates an [oauth.Transport] configured for Google
// OAuth with automatic token refresh.
func NewOAuthTransport(clientID, clientSecret string, creds oauth.Credentials, opts ...oauth.TransportOption) *oauth.Transport {
	defaults := []oauth.TransportOption{
		oauth.WithRefresher(&Refresher{
			ClientID:     clientID,
